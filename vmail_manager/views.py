...
def quotas():
    ...
    for d in domains:
        accounts = db.session.execute(
            db.select(Account)
                .filter_by(domain=d.domain, sendonly=False)
                .order_by(Account.username)
        ).scalars()
        for a in accounts:
            result = subprocess.run(["doveadm", "-f", "flow", "quota", "get", "-u", f"{a.username}@{a.domain}"], capture_output=True, text=True)
            quota_lines = [ line.split(' ') for line in result.stdout.split('\n') if line.startswith('Quota') ]
            lines_splitted = [ [ token.split('=') for token in line if '=' in token ] for line in quota_lines ]
            filtered = [ line_list for line_list in lines_splitted if any(('Type', 'STORAGE') == (pair[0], pair[1]) for pair in line_list) ]
            limit, percent = [ pair[1] for pair in filtered[0] if pair[0] in ('Limit', '%') ]
            domains[d][a] = {'percent': int(percent), 'limit': int(limit) // 1024}
    return render_template('quotas.html', quotas=domains)
